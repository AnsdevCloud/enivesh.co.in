...
import AddBlogBothDatabase, { EditBlogUpdateBothDatabase } from '../../function/firebasecustumfunc/AddDataBothDatabase';
...
const MyEditor = () => {
    ...
    useEffect(() => {
        if (userData) {
            if (userData.role === "Author" || userData.role === "Admin" || userData.role === "Employee") {
                return;
            } else {
                navigate("/blogs");
            }
    ...
    const [contents, setContents] = useState(isEditData);
    ...
    const updatePublish = () => {
        let f = EditBlogUpdateBothDatabase(isEditData?.id, isEditData, contents)
    ...
    useEffect(() => {
        if (isEditData) {
            setContents(isEditData)
            setCoverImageURL(isEditData?.coverImageUrl)
        }
    }, [isEditData])
                    ...
                    <Grid container spacing={2}  >
                        ...
                        <Grid item xs={12}>
                            <Stack p={1} flexDirection={"row"} alignItems={"center"} justifyContent={"space-between"}>
                                ...
                                <Stack flexDirection={"row"} alignItems={"center"} gap={1}>
                                    ...
                                    {isEditData?.id ? <Button onClick={() => updatePublish(contents)} variant='contained' size='small'
                                        disabled={
                                            (contents?.title
                                                && contents?.content
                        ...
                        <Grid item xs={12} sm={10} md={8} >
                            <Paper elevation={0} sx={{ p: 2 }}>
                                ...
                                {(userData?.role === "Author" || userData?.role === "Admin" || userData?.role === "Employee") && <MyTextEditor className="custom-editor" update={editorLoad} data={editorData} onChange={handleEditorChange} />}
